...
import (
	...
	"github.com/amaanq/cr.go/clan"
	"github.com/amaanq/cr.go/labels"
	"github.com/amaanq/cr.go/league"
	"github.com/amaanq/cr.go/location"
	"github.com/amaanq/cr.go/player"
...
func (h *HTTPSessionManager) GetClanRiverRaceLog(ClanTag string) (*clan.CurrentWar, error) {
	...
	var clanwar *clan.CurrentWar
	endpoint := "/clans/" + url.PathEscape(ClanTag) + "/riverracelog"
	data, err := h.Request(endpoint, false)
	if err != nil {
		return clanwar, err
	}
...
func (h *HTTPSessionManager) GetClanCurrentRiverRace(ClanTag string) (*clan.CurrentWar, error) {
	...
	var clanwar *clan.CurrentWar
	endpoint := "/clans/" + url.PathEscape(ClanTag) + "/currentriverrace"
	data, err := h.Request(endpoint, false)
	if err != nil {
		return clanwar, err
	}
