...
func (h *HTTPSessionManager) APILopin(credential LoginCredential) error {
	...
	resp, err := h.Client.R().
		...
		SetResult(&req).
		Post("https://developer.clashroyale.com/api/login")
	if err != nil {
		return err
	}
...
func (h *HTTPSessionManager) GetKeys() error {
	...
	resp, err := h.Client.R().
		SetHeader("Content-Type", "application/json").
		SetResult(&req).
		Post("https://developer.clashroyale.com/api/apikey/list")
	if err != nil {
		return err
	}
	if resp.StatusCode() != 200 {
...
func (h *HTTPSessionManager) AddKey() error {
	...
	body := fmt.Sprintf(`{"name":"%s","description":"%s", "cidrRanges": ["%s"], "scopes": ["royale"]}`, "cr.go", desc, h.IP)
	resp, err := h.Client.R().
		SetHeader("Content-Type", "application/json").
		SetBody(body).
		SetResult(&req).
		Post("https://developer.clashroyale.com/api/apikey/create")
	if err != nil {
		return err
	}
	if resp.StatusCode() != 200 {
...
func (h *HTTPSessionManager) DeleteKey(key Key) error {
	...
	resp, err := h.Client.R().
		...
		SetBody(jsn).
		SetResult(&req).
		Post("https://developer.clashroyale.com/api/apikey/revoke")
	if err != nil {
		return err
	}
	if resp.StatusCode() != 200 {
